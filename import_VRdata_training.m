function [VRdatafinal, VR_tpulsesrel] = import_VRdata_training(mousestr, recnr, posfile, pulsefile, showfigs, savefigs, savetrackingmatfiles)
...
%% Get date of training from file name
trainingday = posfile(1:8);
%assumes pos file is in format yy.mm.dd._Pos_Mx.csv
%%

datadir =  ['/alzheimer/Data_Work/' mousestr '/recordings/'];

%users must specify file names, if not as function param then ask for it in
%dialog box
if  ~exist('pulsefile', 'var') || isempty(pulsefile)
    [pf, pathname] = uigetfile({'*.csv;*.xlsx;*.mat'}, 'Select Sample File to import pulse and reward data from', datadir, 'MultiSelect', 'off');
    if isequal(pf, 0)
        error('User selected Cancel');
    end
    pulsefile = fullfile(pathname, pf);  % Care for the correct type 
end   

%users enter mousenr and recnr and specify file names
if ~exist('posfile', 'var') || isempty(posfile)
    [pf, pathname] = uigetfile({'*.csv';'*.xlsx';'*.mat', }, 'Select Pos File to import POSITION data from', datadir, 'MultiSelect', 'off');
    if isequal(pf, 0)
        error('User selected Cancel');
    end
    posfile = fullfile(pathname, pf);  % Care for the correct type 
end  
...
if makefigs
    if plotball
        ...
        figtitle = sprintf('%s %s VRdata ball %s', mousestr, trainingday, currdate);
        suptitle(figtitle);

        if savefigs     % save the figure in the right directory
            figname = sprintf('/%s_%s_VRdata_XballT_YballT.fig', mousestr, trainingday);
            jpgfigname = sprintf('/%s_%s_VRdata_XballT_YballT.jpg', mousestr, trainingday);
            pngfigname = sprintf('/%s_%s_VRdata_XballT_YballT.png', mousestr, trainingday);
            epsfigname = sprintf('/%s_%s_VRdata_XballT_YballT.eps', mousestr, trainingday);
    ...
    currdate = datestr(date());
    figtitle = sprintf('%s %s VRdata VR %s', mousestr, trainingday, currdate);
    suptitle(figtitle);
    
    if savefigs     % save the figure in the right directory
        figname = sprintf('/%s_%s_VRdata_XVRT_YVRT.fig', mousestr, trainingday);
        jpgfigname = sprintf('/%s_%s_VRdata_XVRT_YVRT.jpg', mousestr, trainingday);
        pngfigname = sprintf('/%s_%s_VRdata_XVRT_YVRT.png', mousestr, trainingday);        
        epsfigname = sprintf('/%s_%s_VRdata_XVRT_YVRT.eps', mousestr, trainingday);
...
if savetrackingmatfiles
    file2save=([mousestr '_' trainingday '_VRdataraw.mat']);
...
if makefigs
    ...
    if plotball    
        ...
        box off;
        currdate = datestr(date());
        figtitle = sprintf('%s %s VRdata VR and ball coords %s', mousestr, trainingday, currdate);
        suptitle(figtitle);
    end
    if savefigs     % save the figure in the right directory
        figname = sprintf('/%s_%s_VRdata_finalXY.fig', mousestr, trainingday);
        jpgfigname = sprintf('/%s_%s_VRdata_finalXY.jpg', mousestr, trainingday);
        pngfigname = sprintf('/%s_%s_VRdata_finalXY.png', mousestr, trainingday);
        epsfigname = sprintf('/%s_%s_VRdata_finalXY.eps', mousestr, trainingday);
...
if savetrackingmatfiles
    file2save=([mousestr '_' trainingday '_VRdatafinal.mat']);
...
if savefigs  
    ...
    figure(hVRfig);
    figname = sprintf('/%s_%s_alltrackingVRfinal.fig', mousestr, trainingday);
    fullname = [figdir figname];
    saveas(gcf, fullname);
    figname = sprintf('/%s_%s_alltrackingVRfinal.eps', mousestr, trainingday);
    fullname = [figdir figname];
    hgexport(gcf, fullname, hgexport('factorystyle'), 'Format', 'eps');
    figname = sprintf('/%s_%s_alltrackingVRfinal.png', mousestr, trainingday);
    fullname = [figdir figname];
    hgexport(gcf, fullname, hgexport('factorystyle'), 'Format', 'png');
    %then plot ball tracking
    if plotball
        figure(hballfig);
        figname = sprintf('/%s_%s_alltrackingballfinal.fig', mousestr, trainingday);
        fullname = [figdir figname];
        saveas(gcf, fullname);
        figname = sprintf('/%s_%s_alltrackingballfinal.eps', mousestr, trainingday);
        fullname = [figdir figname];
        hgexport(gcf, fullname, hgexport('factorystyle'), 'Format', 'eps');
        figname = sprintf('/%s_%s_alltrackingballfinal.png', mousestr, trainingday);
        fullname = [figdir figname];
        hgexport(gcf, fullname, hgexport('factorystyle'), 'Format', 'png');
    end
end
