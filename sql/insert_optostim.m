function insert_optostim(rec_id, time, wavelength, varargin)
%insert_electrostim Insert a new row into the OptoStim table.
%
%   Syntax: insert_optostim(rec_id, time, wavelength, ...)
%
%   [IN]
%       rec_id          :   Recording ID foreign key
%       time            :   Vector (nx1) of stimulation timestamps
%       voltage         :   Vector (nx1) of stimulation wavelength
%       verbose         :   (optional) Verbosity flag, default true
%
%   Example: insert_optostim(1, [100;200;300], [650;650;650]);
%
...
try
    ...
    if ~any(strcmp('OptoStim', tables))
        fprintf('No OptoStim table found.\n')
        return
    end
catch me
    error(me.message);
...
p.addRequired('wavelength', @(x) isnumeric(x) & size(x,2) == 1);
p.addParameter('verbose', true, @islogical);
p.parse(rec_id, time, wavelength, varargin{:});
args = p.Results;

% init query elements
attr = 'rec_id, time, wavelength';
format = '(%d,%f,%f),';
...
% build insert query
insert_query = sprintf('insert into OptoStim(%s) values %s;', attr, vals);
