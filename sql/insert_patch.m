function insert_patch(rec_id, channel, time, amplitude, varargin)
%insert_patch Insert a new row into the Patch table.
%
%   Syntax: insert_patch(rec_id, channel, time, amplitude, ...)
%
%   [IN]
...
%
%   Example: insert_patch(1, 'rec', [100;200;300], [0.3;0.4;0.5]);
%
...
try
    ...
    if ~any(strcmp('Patch', tables))
        fprintf('No Patch table found.\n')
        return
    end
catch me
    error(me.message);
...
% build insert query
insert_query = sprintf('insert into Patch(%s) values %s;', attr, vals);
