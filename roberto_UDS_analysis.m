%% roberto_UDS_analysis



close all %closes all open figures
load('boom') % boom= samples x number of channels matrix with sampling rate:200 (downsampled from raw data)
f=200;
klik=   1%SELECT which channel to use to extract the 1-4 Hz trace
rec_lenght=round(numel(boom(:,1))/f/60);%recording lenght in minutes
spikeStruct = loadKSdir(pwd)



[spikecountovertime]=spikeovertime(spikeStruct.st,0.1,rec_lenght);
spikecountovertime=interp(spikecountovertime,20);spikecountovertime(spikecountovertime<0)=0;

filtered=boom(:,klik);

[s] = spectrogram(filtered,f/100,1,200,f,'yaxis');
plim = abs(s);
ppp=mean(plim(1:4,:));%mean 1-4 hz
gamma=mean(plim(41:101,:));

uppu = sgolayfilt(ppp,3,15);%
 
[ii5,ii6]=FIND_upstatesss(boom,uppu,f,klik,spikecountovertime,rec_lenght);%ii5 and ii6 start stop upstate
%% extra section added 04.02.19 by CH
indices = horzcat(ii5,ii6);
name2save =' US_indices.mat';
save(name2save,'indices')

%%
function [spikecountovertime]=spikeovertime(st,rate,rec_lenght)%rate of sampling,e.g. 0.1 sample every 100ms

[spikecountovertime,edges]=histcounts(st,0:rate:60*rec_lenght); 
spikecountovertime=spikecountovertime/rate;

end
%%
function [ii5,ii6]=FIND_upstatesss(boom,uppu,f,klik,spikecountovertime,rec_lenght,XY)



[ii1,ii2]  = find_upstates(boom,uppu,f,klik,2);
[ii3,ii4]  = find_upstates(boom,spikecountovertime,f,klik,1);

    for i=1:numel(ii3)
    ii34(ii3(i):ii4(i))=1;
    end
    ii34(end+1:ii2(end))=0;
    
    ii5=[];
    ii6=[];
   for i=1:numel(ii1)
     if sum(ii34(ii1(i):ii2(i)))~=0
         ii5 = vertcat(ii5,ii1(i));
         ii6 = vertcat(ii6,ii2(i));       
   end
   end
...
hold on
for i=1: numel(ii5)
rectangle('Position',[ii5(i) -1000 ii6(i)-ii5(i) 2000],'LineStyle', 'none','FaceColor',[246/255 71/255 64/255 0.5]) 
end
% if isempty(XY)~=1
...
hold on
for i=1: numel(ii1(1,:))
rectangle('Position',[ii1(i) 0 ii2(i)-ii1(i) 50],'LineStyle', 'none','FaceColor',[255/255 49/255 46/255 0.5]) 
end
for i=1: numel(ii3(1,:))
...
if exist(ginput2(false,'r*')); %X = select rubbish upstates
    XX=ginput2(false,'r*');
        ...
        ii5(ii5>XX(i-1,1)&ii5<XX(i,1))=[];
        ii6(ii6>XX(i-1,1)&ii6<XX(i,1))=[];
    end
end
   
end
%%
function [ii1,ii2]  = find_upstates(boom,uppu,f,klik,spike_or_freqpower)
%spike_or_freqpower==1   for spike
...
for i=2:numel(ii1)
    if ii1(i)-ii2(i-1)<f/100*35 %if gap between two overthreshold<350 ms FILL IT!
       overthreshold(ii2(i-1):ii1(i))=1;
    end
end
ii1=strfind( [0 overthreshold 0], [ 0 1 ] );    
ii2=strfind( [0 overthreshold 0], [ 1 0 ] );

if spike_or_freqpower==2
    
  for i=1:numel(ii1)
      if ii2(i)-ii1(i)<f/100*10 %no shorter than 
         overthreshold(ii1(i):ii2(i))=0;
      end
  end
  
  for i=1:numel(ii1)
      if ii2(i)-ii1(i)>f*upstatelimit %no shorter than
         overthreshold(ii1(i):ii2(i))=0;
      end
  end    
...
% hold on
% for i=1: numel(ii1(1,:))
% rectangle('Position',[ii1(i) 0 ii2(i)-ii1(i) 500],'LineStyle', 'none','FaceColor',[1 0 0 0.5]) 
% end
end
end
%%
